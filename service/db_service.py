...
model = AutoModelForCausalLM.from_pretrained(
    "/app/OpenVoice/models/llama-3-Korean-Bllossom-8B-MLP-KTLim",
    # quantization_config=quantization_config,
    device_map=device
)
tokenizer = AutoTokenizer.from_pretrained("/app/OpenVoice/models/llama-3-Korean-Bllossom-8B-MLP-KTLim", use_fast=True)
...
voice_young_man = "young_man.wav"
young_man_reference_speaker = f'resources/{voice_young_man}'
target_se_y_man, audio_name_y_man = se_extractor.get_se(young_man_reference_speaker, tone_color_converter, vad=False)

voice_young_woman = "young_woman.wav"
young_woman_reference_speaker = f'resources/{voice_young_woman}'
target_se_y_woman, audio_name_y_woman = se_extractor.get_se(young_woman_reference_speaker, tone_color_converter, vad=False)
...
- 반드시 주어진 단어(Keyword)를 원본 그대로 유지하세요.\n
- 문장 안에 반드시 주어진 단어(Keyword)를 한번만 포함하세요.\n
...
"""}]
    ...
    output = pipe(messages, max_new_tokens=45, do_sample=True, temperature=0.01, top_p=0.98)
...
출력:\n
1. 다양한 프로그램을 실행하기 위해서는 안정적인 [운영체제]가 필요하다.\n
2. 최신 [운영체제]는 보안과 성능 최적화 기능이 강화되어 있다.\n
3. 새로운 소프트웨어를 설치하려면 [운영체제]와의 호환성을 확인해야 한다.\n
...
"""}]
    ...
    output = pipe(messages, max_new_tokens=125, do_sample=True, temperature=0.01, top_p=0.98)
    ...
    if "1." in previous_text:
        start_index = previous_text.find("1.")

        # '1.'부터 마지막까지 슬라이싱합니다.
        previous_text = previous_text[start_index:]
    ...
    previous_text = re.sub(r"[\\\[\]\"';]", "", previous_text) + "."
    previous_text = previous_text.replace("(", " ").replace(")", " ").replace("..", ".").replace(f"{keyword} {keyword}", keyword).strip()
    ...
    previous_dict = {i+1: text for i, text in enumerate(previous_texts)}
    convert_dict = {i+1: text for i, text in enumerate(convert_texts)}
    end = time.time()
    complete_time = end - start
    logging.info(f"실행 시간: {complete_time:.2f}초")

    return {"previous_texts": previous_dict, "convert_texts": convert_dict}
...
def save_tts(voice: str, sentence: str):
    ...
    if voice=="man":
        ...
        target_se = target_se_y_man
        audio_name = audio_name_y_man
    else:
        target_se = target_se_y_woman
        audio_name = audio_name_y_woman
